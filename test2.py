...
import * as THREE from 'https://unpkg.com/three@latest/build/three.module.js';
        ...
        try {
            console.log('Raw particle data string:', container.dataset.particleData);
            const particleData = JSON.parse(container.dataset.particleData);
            console.log('Parsed particle data:', particleData);
            
            if (!Array.isArray(particleData)) {
                throw new Error('Particle data is not an array');
            }

            particleData.forEach(particle => {
                // Add error handling for individual particles
                try {
                    addParticle({
                        ...
                        color: parseInt(particle.color.replace('#', '0x')),
                    });
                } catch (particleError) {
                    console.error('Error adding particle:', particle, particleError);
                }
            });
        } catch (parseError) {
